...
# Convert arrays to comma-separated format for easy parsing
if [ ${#p_cores[@]} -gt 0 ]; then
    p_core_list=$(IFS=,; echo "${p_cores[*]}")
    echo "pcore:${p_core_list}"
fi

if [ ${#e_cores[@]} -gt 0 ]; then
    e_core_list=$(IFS=,; echo "${e_cores[*]}")
    echo "ecore:${e_core_list}"
fi

if [ ${#lpe_cores[@]} -gt 0 ]; then
    lpe_core_list=$(IFS=,; echo "${lpe_cores[*]}")
    echo "lpecore:${lpe_core_list}"
fi
