...
DEFAULT_IMAGE="${basedir}/../../collateral/media/images/coco_448x448.jpg"

usage()
{
    echo "
Usage:
benchmark_genai_vlm.sh -m <model_name> -d <device> -p <precision> [-i <duration_seconds>] [-t <taskset>] [-g <image_path>]

Options:
  -m <name>       Model short name (e.g. minicpm-v-2.6)
  ...
  -g <image>      Path to input image (default: collateral/media/images/coco_448x448.jpg)

Example:
benchmark_genai_vlm.sh -m minicpm-v-2.6 -d GPU -p INT4_SYM_CW -i 60
benchmark_genai_vlm.sh -m gemma-3-4b-it -d GPU -p INT8_ASYM -i 60 -g /path/to/image.jpg
"
}

argparse()
{
    while getopts "hm:d:i:p:t:g:" arg; do
        case $arg in
            m)
            ModelName=${OPTARG}
            ;;
...
# Auto-download VLM test images if default image is missing
if [[ ! -f "${ImagePath}" && "${ImagePath}" == "${DEFAULT_IMAGE}" ]]; then
    echo "[ Info ] VLM test image not found. Downloading..."
    bash "${basedir}/../../tools/genai-downloader/download_images.sh"
fi
[[ -f "${ImagePath}" ]] || { echo "[ Error ] Image not found: ${ImagePath}"; exit 1; }
...
mkdir -p "${DeviceDir}"
Filename="genai-benchmark_${ModelName}_${Device}_${Precision}_vlm_${Timestamp}"
...
echo "[ Info ] Running llm_bench (VLM): model=${ModelName} device=${Device} precision=${Precision} image=${ImagePath##*/} duration=${Duration}s"
...
ImageRes=$(echo "${ImagePath##*/}" | grep -oP '\d+x\d+' || echo "unknown")
echo "[ Info ] Image Resolution: ${ImageRes}"
echo "[ Info ] Type: ${Type^^}"
